    ...
    <div>
      ...
      <div className="two-col">
        <div className="card">
          ...
          <form className="inline-form" onSubmit={e => { e.preventDefault(); void run([...slots, { day, time }]); }}>
          ...
          {sorted.length === 0 ? (
            <div className="empty-state">
              ...
              <button className="btn btn-outline btn-sm" onClick={() => run(SUGGESTED_SLOTS)}>Use suggested weekday schedule</button>
            ...
            <ul className="slot-list">
              {sorted.map(s => (
                <li key={s.id}>
                  ...
                  <button className="notice-close" aria-label={`Remove ${DAY_NAMES[s.day]} ${s.time}`} onClick={() => run(slots.filter(x => x.id !== s.id))}>×</button>
                </li>
              ))}
            </ul>
          )}
